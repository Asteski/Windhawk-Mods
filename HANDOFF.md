...
Per-taskbar/polling refactor (0.6.0): TaskbarState owns each HWND's XAML controls, glyph layers, native style/geometry backups, tooltip/menu state, size handlers, timers, Bluetooth async queries, and click suppression. UI delegates capture their owner and establish a thread-local scope, including delayed callbacks. Primary and secondary windows are enumerated; optional CSecondaryTaskBand host symbols and CSecondaryTray initialization hook support secondary roots/hotplug. A taskbar window subclass releases state on WM_NCDESTROY; disable/settings reapply release XAML on the owning thread. Middle-click suppression now has five entries, fixing Battery's previous out-of-bounds index. Network and Bluetooth availability are cached for 2 seconds per taskbar; Bluetooth device-name queries also use 2 seconds. Rendering, sound, and battery retain the 500 ms timer; airplane-mode menu changes invalidate local slow caches. Native styles are still captured independently per taskbar; no new explicit native-template override was introduced.

Validation: syntax check and Windhawk CLI compilation passed. Build 0.6.0_354577 loaded in Explorer PID 18360; UI Automation found Sound, Bluetooth, Network, Battery with live state and expected 56 physical-pixel height. Only primary Shell_TrayWnd is available, so secondary-monitor/hotplug behavior remains unverified. Replacing the prior build logged a crash in OLD DLL 0.6.0_481342 at 2026-09-13 04:25:33; do not attribute that fault to the new DLL. New build's disable/enable cycle preserved Explorer PID 18360. Editor synchronized.
