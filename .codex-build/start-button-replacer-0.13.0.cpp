...
Images switch automatically when the Start button's actual taskbar theme changes, including Windows custom mode with different Windows and app themes. Image size, GIF playback and effects are shared between themes. Existing state fallback rules still apply after selecting the theme's sources.
...
void DetachInstance(const std::shared_ptr<StartIconInstance>& instance) {
    ...
    if (startButton && instance->themeChangedAttached) {
        try {
            startButton.ActualThemeChanged(instance->themeChangedToken);
        } catch (...) {
...
bool AttachToStartButton(const wux::FrameworkElement& startButton,
    ...
    auto imageTheme = startButton.ActualTheme();
    ...
    try {
        ...
        std::weak_ptr<StartIconInstance> weakInstance = instance;
        instance->themeChangedToken = startButton.ActualThemeChanged(
            [weakInstance](const wux::FrameworkElement& sender,
                           const wf::IInspectable&) {
