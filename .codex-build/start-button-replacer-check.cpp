// ==WindhawkMod==
// @id              start-button-replacer-fork
// @name            Start Button Replacer - Fork
...
Configure **Light theme images** and **Dark theme images** to override the normal, hover, pressed and activated sources for each theme. Blank overrides inherit the corresponding source from **Images**, preserving existing configurations. The normal image in **Images** remains required as a fallback.

Images switch automatically within about one second of changing Windows mode in Settings > Personalization > Colors. The mod reads the system theme directly, so it also works when the taskbar's XAML theme does not change. In custom mode, it follows Windows mode, not app mode. Image size, GIF playback and effects are shared between themes. Existing state fallback rules still apply after selecting the theme's sources.
...
- lightThemeImages:
  - imageSource: ""
    $name: Normal image source
    $description: >-
      Image for the light taskbar theme. Leave blank to use the corresponding source from Images. Supports the same paths and URLs.
  - hoverImageSource: ""
    $name: Hover image source
    $description: >-
      Image for the light taskbar theme. Leave blank to use the corresponding source from Images. Supports the same paths and URLs.
  ...
  - activatedImageSource: ""
    ...
    $description: >-
      Image for the light taskbar theme. Leave blank to use the corresponding source from Images. Supports the same paths and URLs.
  $name: Light theme images
  $description: Optional image overrides applied automatically for the light taskbar theme.

- darkThemeImages:
  - imageSource: ""
    $name: Normal image source
    $description: >-
      Image for the dark taskbar theme. Leave blank to use the corresponding source from Images. Supports the same paths and URLs.
  - hoverImageSource: ""
    $name: Hover image source
    $description: >-
      Image for the dark taskbar theme. Leave blank to use the corresponding source from Images. Supports the same paths and URLs.
  ...
  - activatedImageSource: ""
    ...
    $description: >-
      Image for the dark taskbar theme. Leave blank to use the corresponding source from Images. Supports the same paths and URLs.
  $name: Dark theme images
  $description: Optional image overrides applied automatically for the dark taskbar theme.
...
struct StartIconInstance {
    ...
    wux::DispatcherTimer themeTimer{nullptr};
    winrt::event_token themeChangedToken{};
    bool themeChangedAttached = false;
    wux::ElementTheme imageTheme = wux::ElementTheme::Default;
...
ModSettings LoadSettings() {
    ...
    constexpr const wchar_t* imageKeys[] = {
        L"imageSource", L"hoverImageSource", L"pressedImageSource",
        L"activatedImageSource",
    };
    static_assert(ARRAYSIZE(imageKeys) == kIconStateCount);
    for (size_t i = 0; i < kIconStateCount; i++) {
        settings.lightThemeImages[i] = GetStringSetting(
            (std::wstring(L"lightThemeImages.") + imageKeys[i]).c_str());
        settings.darkThemeImages[i] = GetStringSetting(
            (std::wstring(L"darkThemeImages.") + imageKeys[i]).c_str());
    }
...
void DetachInstance(const std::shared_ptr<StartIconInstance>& instance) {
    ...
    if (instance->themeTimer) {
        try {
            instance->themeTimer.Stop();
            if (instance->themeChangedAttached) {
                instance->themeTimer.Tick(instance->themeChangedToken);
            }
        } catch (...) {
            Wh_Log(L"Removing theme-change handler failed");
        }
    }
    instance->themeTimer = nullptr;
    instance->themeChangedAttached = false;
    instance->themeChangedToken = {};
...
wux::ElementTheme GetSystemImageTheme() {
    DWORD lightTheme = 0;
    DWORD size = sizeof(lightTheme);
    LSTATUS status = RegGetValueW(
        HKEY_CURRENT_USER,
        L"Software\\Microsoft\\Windows\\CurrentVersion\\Themes\\Personalize",
        L"SystemUsesLightTheme", RRF_RT_REG_DWORD, nullptr, &lightTheme, &size);
    if (status != ERROR_SUCCESS) {
        // Default means unknown: retain the last theme and retry on the next
        // timer tick instead of switching images after a transient read error.
        return wux::ElementTheme::Default;
    }
    return lightTheme ? wux::ElementTheme::Light : wux::ElementTheme::Dark;
}
...
bool AttachToStartButton(const wux::FrameworkElement& startButton,
    ...
    auto imageTheme = GetSystemImageTheme();
    if (imageTheme == wux::ElementTheme::Default) {
        imageTheme = startButton.ActualTheme();
        Wh_Log(L"System theme unavailable; using the initial XAML theme");
    }
    ApplyThemeImageSources(settings, imageTheme);
    ...
    try {
        ...
        instance->imageTheme = imageTheme;
        instance->hovering = initiallyHovering;
        ...
        std::weak_ptr<StartIconInstance> weakInstance = instance;
        // Explorer can theme the Start icon without changing ActualTheme on
        // the button itself. Observe the Windows setting independently.
        // DispatcherTimer keeps all XAML access and cleanup on the owner thread.
        instance->themeTimer = wux::DispatcherTimer();
        instance->themeTimer.Interval(std::chrono::seconds(1));
        instance->themeChangedToken = instance->themeTimer.Tick(
            [weakInstance](const wf::IInspectable&,
                           const wf::IInspectable&) {
                auto current = weakInstance.lock();
                if (!current || current->detached || g_unloading) {
                    return;
                }
                try {
                    auto theme = GetSystemImageTheme();
                    if (theme == wux::ElementTheme::Default ||
                        theme == current->imageTheme) {
                        return;
                    }
                    auto sender = current->startButton.get();
                    if (!sender) {
                        UnregisterInstance(current);
                        DetachInstance(current);
                        return;
                    }
                    Wh_Log(L"Windows theme changed to %s",
                           theme == wux::ElementTheme::Light ? L"light" : L"dark");
                    ModSettings next = GetSettingsSnapshot();
                    ApplyThemeImageSources(next, theme);
                    bool sourcesChanged = false;
                    for (size_t i = 0; i < kIconStateCount; i++) {
                        auto state = static_cast<IconState>(i);
                        sourcesChanged |= GetImageSourceForState(next, state) !=
                            GetImageSourceForState(current->settings, state);
                    }
                    current->imageTheme = theme;
                    if (!sourcesChanged) {
                        return;
                    }
                    // Rebuild on the owning XAML thread. Detachment revokes
                    // old image callbacks before the new set begins loading.
                    bool hovering = current->hovering;
                    DetachInstanceForStartButton(sender);
                    AttachToStartButton(sender, hovering);
                } catch (const winrt::hresult_error& e) {
                    Wh_Log(L"Applying theme images failed: 0x%08X %s",
                           e.code().value, e.message().c_str());
                } catch (...) {
                    Wh_Log(L"Applying theme images failed");
                }
            });
        instance->themeChangedAttached = true;
        instance->themeTimer.Start();
        Wh_Log(L"Theme image monitor started: %s",
               imageTheme == wux::ElementTheme::Light ? L"light" : L"dark");

        if (!LoadImages(instance)) {
            // Keep the theme listener alive so another theme can recover
            // even when this theme's main source cannot be constructed.
