// ==WindhawkMod==
// @id              block-windows-startmenu-and-hosts-fork
// @name            Block Start Menu and Hosts - Fork
...
*   **CrossDeviceResume.exe**           -> The Cross Device Resume process.
*   **CrossDeviceService.exe**          -> The Cross Device Service process.
...
It then places a hook on the `CreateProcessInternalW` function: If the system attempts to launch any of the blocked .exe (example: user press WIN key), the hook creates the target suspended and terminates it before any code runs, preventing the launch without surfacing an access-denied prompt to the caller.  
...
- BlockCrossDeviceResume: false
  $name: Block CrossDeviceResume.exe
  $description: Kills existing instances and blocks future launches of Cross Device Resume.
- BlockCrossDeviceService: false
  $name: Block CrossDeviceService.exe
  $description: Kills existing instances and blocks future launches of Cross Device Service.
*/
// ==/WindhawkModSettings==
...
bool g_blockCrossDeviceResume = false;
bool g_blockCrossDeviceService = false;
...
void LoadSettings() {
    ...
    g_blockCrossDeviceResume          = Wh_GetIntSetting(L"BlockCrossDeviceResume");
    g_blockCrossDeviceService         = Wh_GetIntSetting(L"BlockCrossDeviceService");
}
...
void TerminateConfiguredProcesses() {
    ...
    if (g_blockCrossDeviceResume)          TerminateProcessByName(L"CrossDeviceResume.exe");
    if (g_blockCrossDeviceService)         TerminateProcessByName(L"CrossDeviceService.exe");
}
...
BOOL QuietlyBlockProcessLaunch(
    ...
    LPPROCESS_INFORMATION lpPI, PHANDLE hNewToken)
{
    ...
    LPPROCESS_INFORMATION processInfo = lpPI ? lpPI : &localPI;
    DWORD quietFlags = dwFlags | CREATE_SUSPENDED;

    BOOL created = CreateProcessInternalW_Original(
        hToken, lpAppName, lpCmdLine, lpPA, lpTA, bInherit, quietFlags, lpEnv,
        lpDir, lpSI, processInfo, hNewToken);
    ...
    LPPROCESS_INFORMATION lpPI, PHANDLE hNewToken)
{
    ...
    bool block =
        ...
        (g_blockCrossDeviceResume          && matches(L"CrossDeviceResume.exe"))           ||
        (g_blockCrossDeviceService         && matches(L"CrossDeviceService.exe"));

    if (block) {
        Wh_Log(L"Blocked launch: AppName=%ls, CmdLine=%ls",
            lpAppName ? lpAppName : L"NULL",
            lpCmdLine ? lpCmdLine : L"NULL");
        return QuietlyBlockProcessLaunch(hToken, lpAppName, lpCmdLine, lpPA, lpTA,
            bInherit, dwFlags, lpEnv, lpDir, lpSI, lpPI, hNewToken);
    }
