// ==WindhawkMod==
// @id              explorer-ctrlaltn-newfile
// @name            Explorer Ctrl+Alt+N to New File
// @name:zh-CN      用 Ctrl+Alt+N 创建新文件
// @description     Swallow Ctrl+Alt+N in Explorer and create a new file in-place, including desktop.
// @version         1.1
// @author          lieyanbang
...
Intercepts Explorer’s **Ctrl+Alt+N** (open new window) and replaces it with creating an empty file in the current directory, then entering rename mode.

**How it works**

Uses Windhawk to inject a `WH_KEYBOARD_LL` low-level keyboard hook into explorer.exe to intercept Ctrl+Alt+N.
...
拦截资源管理器的 **Ctrl+Alt+N**（新建窗口操作），改为在当前目录创建空文件并重命名。

**实现原理**

通过 Windhawk 在 explorer.exe 注入 `WH_KEYBOARD_LL` 低级键盘钩子拦截Ctrl+Alt+N。
...
static void PerformNewFileAction() {
    ...
    if (!target) {
        Wh_Log(L"[ctrlaltn] No explorer window detected.");
        return;
    }
    ...
    if (!CreateEmptyFile(newFile)) {
        Wh_Log(L"[ctrlaltn] Failed to create file: %ls", newFile.c_str());
        return;
    }
...
static BOOL KeybdHook_Init() {
    ...
    if (!g_lowLevelHook) {
        Wh_Log(L"[ctrlaltn] SetWindowsHookEx failed.");
...
static LRESULT CALLBACK LowLevelKeybdProc(int nCode, WPARAM wParam, LPARAM lParam) {
    if (nCode == HC_ACTION) {
        ...
        if (info && isKeyUp && (info->vkCode == VK_LCONTROL || info->vkCode == VK_RCONTROL ||
                                info->vkCode == VK_LMENU || info->vkCode == VK_RMENU))
            g_nKeyDown = false;
