...
static void ComputeLayout(HMONITOR hMon) {
    ...
    int thumbTopOffset = padTop + ((ThumbnailIsBottom() && g_settings.showThumbnails) ? headerAndDividerH : 0);
    int initialTop  = elemPadTop + thumbTopOffset;
    int baseContentH = sidePlacement ? std::max(thumbH, rowTitleH) : thumbH;
    int bottomInc   = (baseContentH + padBot) + elemPadBot + initialTop + (ThumbnailIsTop() ? headerAndDividerH : 0);
