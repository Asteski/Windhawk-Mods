// ==WindhawkMod==
// @id              taskbar-elastic-pill-fork
// @name            Taskbar Elastic WinUI Pill - Fork
...
- Pill:
  ...
  - Margins: "0, 5"
    $name: Pill Position (Horizontal, Bottom)
    $description: Horizontal position and bottom margin. In grouped mode, horizontal is an offset from center. In ungrouped mode, horizontal is distance from the task button's left edge.
  - UngroupedTaskMode: false
    $name: Ungrouped task mode
    $description: Anchor the pill from the left edge of each task button, useful when taskbar labels make buttons different widths.
  - PillRadius: '2'
...
struct PillContext {
    ...
    std::atomic<bool> layoutUpdatePending{false};
...
void LoadSettings() {
    ...
    g_settings.UngroupedTaskMode = Wh_GetIntSetting(L"Pill.UngroupedTaskMode") != 0;
    ...
    Settings const& localSettings)
{
    ...
    float layoutX = static_cast<float>(g.Padding().Left + g.BorderThickness().Left);
    float targetAbsoluteX = localSettings.UngroupedTaskMode
        ? (float)point.X
        : (float)(point.X + (b.ActualWidth() / 2.0) - (pillW / 2.0));
    float targetX = std::round(targetAbsoluteX) - layoutX;
...
void EnsurePillAndPosition(winrt::Windows::UI::Xaml::FrameworkElement const& button, bool isActive, const Settings& localSettings) {
    ...
    float lastW = -1.0f, lastH = -1.0f, lastR = -1.0f, lastMB = -1.0f, lastMH = -1.0f, lastUngroupedMode = -1.0f;
    ...
    visual.Properties().TryGetScalar(L"SettingsUngroupedMode", lastUngroupedMode);
    ...
    if (std::abs(lastW - (float)settingsToUse.PillWidth) > 0.001f || 
        ...
        std::abs(lastUngroupedMode - (settingsToUse.UngroupedTaskMode ? 1.0f : 0.0f)) > 0.001f) {
        ...
        visual.Properties().InsertScalar(L"SettingsUngroupedMode", settingsToUse.UngroupedTaskMode ? 1.0f : 0.0f);
        ...
        pill.Margin({(double)settingsToUse.PillMarginHorizontal, 0, 0, (double)settingsToUse.PillMarginBottom});
    }
    ...
    std::shared_ptr<PillContext> ctx = nullptr;
    {
        ...
        if (it != g_pillContexts->end()) {
            ...
            ctx->layoutToken = grid.LayoutUpdated([weakCtx, weakGrid](auto const&, auto const&) {
                ...
                if (context->layoutUpdatePending.exchange(true)) return;

                auto dispatcher = g.Dispatcher();
                if (!dispatcher) {
                    context->layoutUpdatePending.store(false);
                    return;
                }
                dispatcher.RunIdleAsync([weakCtx, weakGrid](winrt::Windows::UI::Core::IdleDispatchedHandlerArgs const&) {
                    if (g_unloading) return;
                    auto context = weakCtx.lock();
                    auto g = weakGrid.get();
                    if (!context || !g) return;
                    ...
                    if (active && p) {
                        ...
                        } catch (...) { Wh_Log(L"Exception in deferred LayoutUpdated handler"); }
                    }
                });
            });
        }
